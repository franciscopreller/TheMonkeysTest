...
```
<VirtualHost *>
    DocumentRoot "/path/to/TheMonkeysTest/build/public/"
    ServerName TheMonkeysTest.dev
    <Directory /path/to/TheMonkeysTest/build/public/>
        AllowOverride All
        Order allow,deny
        Allow from all
    </Directory>
...
To avoid any extra configuration, create a database called `TheMonkeysTest` and a user called
`TheMonkeysTest` with password `TheMonkeysTest` with `SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, INDEX, ALTER`
permissions on the `TheMonkeysTest` database, with access from localhost.
...
Visit [http://TheMonkeysTest.dev/](http://TheMonkeysTest.dev/) and you should see the home page. Check your browser's error console
for any load errors or javascript errors.

Visit [http://TheMonkeysTest.dev/tests/](http://TheMonkeysTest.dev/tests/) to run the automated acceptance tests.
